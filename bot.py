        ...
        while True:
            ...
            try:
                await user_bot.edit_message(
                    int(chnl_id),
                    msg_id,
                    "**RandomLoops Bots Stats.**\n\n`Performing a periodic check...`",
                )
            except MessageNotModifiedError:
                pass
            c = 0
            edit_text = "**RandomLoops Bots Stats.**\n\n"
            for bot in bots:
                try:
            ...
            edit_text += f"\n**Last Checked:** \n`{t} - {day} {month} {year} [IST]`\n\n__Bots status are " \
                         f"auto-updated every 2 hours__"
            await user_bot.edit_message(int(chnl_id), int(msg_id), edit_text)
...
try:
    user_bot.loop.run_until_complete(run())
    user_bot.disconnect()  # try to prevent AuthKeyDuplicatedError
